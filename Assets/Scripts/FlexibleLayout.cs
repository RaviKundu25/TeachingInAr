...
public class FlexibleLayout : LayoutGroup
{
    ...
    public void clearAndChangeScreen()
    {
        GameObject GC = GameObject.Find("GameController");
        GameObject usersNotSelected = GameObject.Find("UsersNotSelected");
        GameObject listContent = GameObject.Find("ListContent");
        for (int i = 0; i < transform.childCount; i++)
        {
            transform.GetChild(i).gameObject.SetActive(false);
            transform.GetChild(i).parent = usersNotSelected.transform;
        }
        for (int i = 0; i < listContent.transform.childCount; i++)
        {
            if (GC.GetComponent<AgoraManager>().selectedUsers.Contains(listContent.transform.GetChild(i).name))
            {
                for (int j = 0; j < usersNotSelected.transform.childCount; j++)
                {
                    if(usersNotSelected.transform.GetChild(j).name == listContent.transform.GetChild(i).name)
                    {
                        usersNotSelected.transform.GetChild(j).gameObject.SetActive(true);
                        usersNotSelected.transform.GetChild(j).parent = transform;
                    }
                }
            }
        }
